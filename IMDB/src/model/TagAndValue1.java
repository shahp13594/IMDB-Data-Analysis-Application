...
public class TagAndValue1   extends Observable{
	...
	public TagAndValue1(Director d1) {
	...
	private ResultSet executeQuery(Connection con2) {
		...
		if (d.getDirectorSelectedList().get(0) == "ALL") {
			...
			QueryStatmentTagMid += prevQuery;
			QueryStatmentTagMid += "\nINTERSECT\n";
			for (k = 0; k < d.getDirectorSelectedList().size() - 1; k++) {

				QueryStatmentTagMid += "SELECT D.MID \n FROM MOVIE_DIRECTORS D WHERE D.DIRECTORNAME = '"
						+ d.getDirectorSelectedList().get(k) + "' \n" + andOrAttribute + " \n";
			...
			QueryStatmentTag += "SELECT T.TID , T.TVALUE \n FROM TAGS T , MOVIE_TAGS MT \n WHERE MT.TID = T.TID AND MT.MID IN ( "
					+ QueryStatmentTagMid + " )";

			System.out.println(
					"------------------------------tag mid selected----------------------------------------------------------");
			System.out.println(QueryStatmentTagMid);
			System.out.println(
					"-------------------------------------------------------------------------------------------");
		}
